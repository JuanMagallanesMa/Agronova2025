...
typedef SaveFunction = Future<void> Function(ReferenciaBase item);

class RegistroCatalogo<T extends ReferenciaBase> extends StatefulWidget {
  final T? item;
  final String title;
  final SaveFunction onSave;
...
class _RegistroCatalogoState<T extends ReferenciaBase>
  ...
  Future<void> _saveForm() async {
    ...
    // 1. Crear el ReferenciaBase
    final itemToSave = ReferenciaBase(
      // Corregido: 'id' no puede ser null, asignamos '' si es nuevo
      id: widget.item?.id ?? '',
      nombre: _nombre,
      // Corregido: Usamos AppStatus y el estado existente
      estado: widget.item?.estado ?? AppStatus.activo,
    );

    try {
      // 2. Enviar el ReferenciaBase DIRECTAMENTE a la función onSave
      // (Sin 'as T' y sin 'widget.provider')
